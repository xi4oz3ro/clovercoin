...
#if defined(NDEBUG)
# error "Clovercoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xc1f7ee75f2f279812235d9b772c3649df11f89e550300a13a6645875ce0a9108");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 10); // Clovercoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Clovercoin Signed Message:\n";
...
{
    // Clovercoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    else
    {
     ...
     // Subsidy is cut in half every 960000 blocks, which will occur approximately every 4 years
     nSubsidy >>= (nHeight / 960000); // Clovercoin: 840k blocks in ~4 years

     return nSubsidy + nFees;
    }
}

static const int64 nTargetTimespan = 10 * 60; // Clovercoin: 10 minutes
static const int64 nTargetSpacing = 1 * 30; // Clovercoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Clovercoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Clovercoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xc1f7ee75f2f279812235d9b772c3649df11f89e550300a13a6645875ce0a9108");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "[GOLBAL TIMES:20170906]Xi chairs summit to set course for next golden decade of BRICS";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("999984710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1504627200;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 1208732;
        ...
        assert(block.hashMerkleRoot == uint256("0x721470d73fd151875653e0b223dcc29a50a11da715ce8f6af47040a1aba97252"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Clovercoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("ClovercoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("ClovercoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static ClovercoinMiner(CWallet *pwallet)
{
    printf("ClovercoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("clovercoin-miner");
    ...
    try { loop {
        ...
        printf("Running ClovercoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("ClovercoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&ClovercoinMiner, pwallet));
}
